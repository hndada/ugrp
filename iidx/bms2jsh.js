...
lv=["™H","š‚P","š‚Q","š‚R","š‚S","š‚T","š‚U","š‚V","š‚W","š‚X",
	"š10","š11","š12","š‚V".fontcolor("red"),"š‚W".fontcolor("red")];
...
function scrl_body(incode){
	...
	switch(incode){
		case 27:		// [ESC]
			editmode ^= 1;
			if (editmode) alert("editƒ‚[ƒh‚É‚È‚è‚Ü‚·");
			else		  alert("editƒ‚[ƒh‚ð‰ðœ‚µ‚Ü‚·");
			break;
		case 16:	// [SHIFT]
			sftkey ^= 1;
			break;
		case 74:	// url copy [j]
			if (window.confirm("ƒNƒŠƒbƒvƒ{[ƒh‚ÉƒŠƒ“ƒN‚ðƒRƒs[‚µ‚Ü‚·B")) {
...
function rndjump(rstr1,rstr2,rndopt,rndval){
	...
	for(ri=0;ri<8;ri++)
		if(rstr1.indexOf(""+ri)<0 ||
		   rstr2.indexOf(""+ri)<0){alert("“ü—Í‚ª³‚µ‚­‚ ‚è‚Ü‚¹‚ñB");return;}
			...
			for(cni=cn[h][n].length-1; cni>=0; cni--){
				...
				for (cnj=0; cnj<(cnz<10 ? 1:2); cnj++) {
					...
					if(!legacy) {
						...
						objstr=imgdir+(hcn ? "h":"l")+cob[jpos]+dstr;
...
function stat_result()
{
	...
	for (var sh=0; sh<2; sh++) {
		...
		oaistr=["—‘Å","“¯Žž","ŠK’i","ƒgƒŠ","c˜A","@ŽM","‚b‚m"];
...
//			oa+= "‡Œv "+(Math.round(1000 * oattl/100)/10)+" pt\n";
		}
	}
